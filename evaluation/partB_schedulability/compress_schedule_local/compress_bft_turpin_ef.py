...
def Main(filename):
    ...
    filter_us        = GetTimesUs(file_data, LOG_APP_FILTER_SENSOR)
    agree_us         = GetTimesUs(file_data, LOG_APP_AGREEMENT)
    source_sel_us    = GetTimesUs(file_data, LOG_APP_SOURCE_SELECT)
    compute_us       = GetTimesUs(file_data, LOG_APP_COMPUTE_0)
    ...
    filter_slots        = GetSlots(filter_us)        
    ...
    if (PRINT_HIST):
        ...
        PrintHist("Filtering", filter_slots)
    ...
    # Determine how many slots each activity takes.
    max_sensor_read_slots   = max(sensor_read_slots)
    max_filter_slots        = max(filter_slots)
    ...
    replica_filter_slot        = replica_read_sensor_slot + max_sensor_read_slots
    replica_agree_slot         = replica_filter_slot + max_filter_slots
    replica_source_sel_slot    = replica_agree_slot + max_agree_slots
    replica_compute_slot       = replica_source_sel_slot + max_source_sel_slots
    ...
    with open("%s/max_slots.txt" % (output_dir), 'w') as f:
        f.write('%d, %d, %d' % (max_agree_slots, max_filter_slots, 0))
    ...
    f = open(output_dir + "/custom_replica_table_bft_turpin_ef.c","w+")
    f.write(boilerplate)
    for minor in range(0, MINOR_FRAME_COUNT):    
        ...
        elif (minor == replica_filter_slot):
            f.write("REPLICA_FILTER_SENSOR_MID, \n")      
        elif (minor == replica_agree_slot):
            f.write("REPLICA_START_AGREEMENT_MID, \n")                       
    ...
    f = open(output_dir + "/custom_sim_table_bft_turpin_ef.c","w+")
    f.write(boilerplate)
    for minor in range(0, MINOR_FRAME_COUNT):    
        f.write("/* slot %d */\n" % minor)
