...
def Main(filename):
    ...
    filter_us        = GetTimesMultiUs(file_data, LOG_APP_PARALLEL_START, [LOG_APP_FILTER_0])
    ...
    replica_agree_slot         = replica_compute_slot + max_filter_slots
    replica_source_sel_slot    = replica_agree_slot + max_agree_slots  

    if (max_compute_slots > (max_filter_slots + max_agree_slots + max_source_sel_slots)):   
        replica_send_actuator_slot = replica_compute_slot + max_compute_slots
    else:
        replica_send_actuator_slot = replica_compute_slot + (max_filter_slots + max_agree_slots + max_source_sel_slots)     
    ...
    with open("%s/max_slots.txt" % (output_dir), 'w') as f:
        f.write('%d, %d, %d' % (max_agree_slots, max_filter_slots, max_state_disp_slots))
    ...
    f = open(output_dir + "/custom_replica_table_igor_ef.c","w+")
    f.write(boilerplate)
    ...
    f = open(output_dir + "/custom_sim_table_igor_ef.c","w+")
    f.write(boilerplate)
    for minor in range(0, MINOR_FRAME_COUNT):    
        f.write("/* slot %d */\n" % minor)
