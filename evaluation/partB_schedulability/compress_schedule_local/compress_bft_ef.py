...
def Main(filename):
    ...
    replica_agree_slot         = replica_read_sensor_slot + max_sensor_read_slots
    ...
    with open("%s/max_slots.txt" % (output_dir), 'w') as f:
        f.write('%d, %d, %d' % (max_agree_slots, 0, 0))
    ...
    f = open(output_dir + "/custom_replica_table_bft_ef.c","w+")
    f.write(boilerplate)
    ...
    f = open(output_dir + "/custom_sim_table_bft_ef.c","w+")
    f.write(boilerplate)
    for minor in range(0, MINOR_FRAME_COUNT):    
        f.write("/* slot %d */\n" % minor)
