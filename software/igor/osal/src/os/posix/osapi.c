                      ...
                      uint32 flags, int core_id)
{
    ...
    /* Used for pinning tasks to cores. */
    int pthread_ret;
    cpu_set_t cpuset;
    ...
    /* Use the priority in the startup script verbatim. */
    os_priority = priority - 1;
    ...
    */
    if (core_id == 0) { /* do not pin thread */
        printf("OS_TaskCreate: Not pinning task %s to any core\n", task_name);
    } else { /* pin the thread */
        CPU_ZERO(&cpuset);
        CPU_SET(core_id - 1, &cpuset);
        pthread_ret = pthread_setaffinity_np(OS_task_table[possible_taskid].id, sizeof(cpu_set_t), &cpuset);
        if (pthread_ret == 0) {
            printf("OS_TaskCreate: Successfully pinned task %s to core %d\n", task_name, core_id);
        } else {
            printf("OS_TaskCreate: Error pinning task %s to core %d [ret = %d]\n", task_name, core_id, pthread_ret);
        }    
    }
